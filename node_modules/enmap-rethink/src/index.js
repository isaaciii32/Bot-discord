...
class EnmapProvider {
  ...
  async init(enmap) {
    ...
    try {
      ...
      throw `Unable to initialized enmap-rethink: ${err}`;
    }
    ...
    feed.each((err, change) => {
      ...
      if (newVal) {
        let parsedValue = newVal.data;
        if (newVal.data[0] === '[' || newVal.data[0] === '{') {
          parsedValue = JSON.parse(newVal.data);
        }
        Map.prototype.set.call(enmap, newVal.id, parsedValue);
      } else {
  ...
  async fetch(key) {
    ...
    if (row) this.enmap.set(key, parsedValue);
  ...
  */
  async fetchEverything() {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      Map.prototype.set.call(this.enmap, row.id, parsedValue);
    }
    return this;
  }
  /**
